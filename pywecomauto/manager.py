...
from sqlalchemy import create_engine, exists, asc, not_, or_, desc
from sqlalchemy.orm import sessionmaker

from pywecomauto.models import Task, Client, ClientStatus, TaskStatus, TaskType, Notification, App, now

DIR = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(DIR, r'db.db3')
ENGINE = create_engine(f"sqlite:///{DB_PATH}", connect_args={"check_same_thread": True}, echo=False)
print('manager.py',ENGINE)
...
def get_next_task():
    ...
    task = db.query(Task) \
        ...
        .filter(Task.status == TaskStatus.CREATED.value) \
        .filter(Task.deleted == 0) \
        .order_by(desc(Task.priority), asc(Task.scheduled_time)) \
        .first()
    db.close()
    return task
